...
wyxClient.prototype.checkSignature = function(queryObj, secret) {
  ...
  var sig = sign.buildSign(baseStr, secret || this.secret, 'sha1');
...
wyxClient.prototype.authorize = function (req, res, callback, context) {
  ...
  // if authorized
  if(this.user.userId){
    return callback.call(context, null, { platform: this.platform, data: this.user, expire: this.expire });
  }
  ...
  var basestring = sign.sortQueryToString(param); 
  var hash = sign.buildSign(basestring, this.secret, 'sha1');
  if(hash !== signature){
  ...
  var user = { platform: this.platform, data: this.user, expire: this.expire };
...
wyxClient.prototype.appfriends_info = function (param, callback, context) {
  param = param || {};
  var self = this;
