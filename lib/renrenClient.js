...
renrenClient.prototype.authorize = function (req, res, callback, context) {
	...
	var access_cbfunc = function(error, access_token, refresh_token, result) {
		...
		if(result['expires_in']){
			...
			expire_at.setTime(expire_at.getTime()+result['expires_in']*1000);
		}
...
renrenClient.prototype.geneCommonUser = function (data) {
	...
	return {
  		...
  		name: data.name,
  		location:{
  			country:	data.hometown_location.country,
  			province: data.hometown_location.province,
  			city: 		data.hometown_location.city,
  		},
  		imageUrl: data.tinyurl,
  		imageUrl_large: data.mainurl,
  		gender:	data.sex == 1,
  		is_special:	Boolean(data.zidou == 1),
			specialtype: Number(data.vip),
  		lang: "zh-cn",
  	};
};
...
renrenClient.prototype.account_info = function (param, callback, context) {
	param = param || {};
	param.fields = "uid,name,sex,star,zidou,vip,tinyurl,mainurl,hometown_location";
	
	ClientBase.prototype.account_info.call(this, param, callback, context);
};