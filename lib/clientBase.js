 ...
 * @param {Object}
 */
clientBase.prototype._buildBaseString = function buildBaseString(paramObj, useRawValue, withoutsep, clearPrefix, exclud) {
  if(!paramObj) return "";

  var argument_pairs= this._makeArrayOfArgumentsHash(paramObj, clearPrefix, exclud);
...
clientBase.prototype._makeArrayOfArgumentsHash= function(argumentsHash, clearPrefix, exclud) {
  var argument_pairs= [];
  for(var key in argumentsHash ) {
    ...
    if(clearPrefix){
      if(key.indexOf(clearPrefix) === 0){
        key = key.substring(clearPrefix.length);
      } else {
        continue;
      }
    }
