 ...
 * build base string
 * @param {Object}
 * @param {Boolean} useRawValue if not true encode the param first
 * @param {Boolean} withoutsep if true without &, eles add a &
 * @param {String} prefix string like 'wyx_', only this will be use to buildBaseString
 * @param {Boolean} clearPrefix if true, will cut the prefix First, e.g. rr_sig_ should remove
 * @param {String} exclud exclud field, e.g. 'wyx_signature'
 */
clientBase.prototype._buildBaseString = function buildBaseString(paramObj, useRawValue, withoutsep, prefix, clearPrefix, exclud) {
  if(!paramObj) return "";

  var argument_pairs= this._makeArrayOfArgumentsHash(paramObj, prefix, clearPrefix, exclud);
...
clientBase.prototype._makeArrayOfArgumentsHash= function(argumentsHash, prefix, clearPrefix, exclud) {
  var argument_pairs= [];
  for(var key in argumentsHash ) {
    if(key === exclud) continue;
    var value= argumentsHash[key];
    // clear prefix like 'xn_sig_'
    if (prefix) {
